    ...
    const fetchLeaderboard = async () => {
      try {
        ...
        for (const i in challenges) {
          const challengeLeaderboard = await fetchChallengeLeaderboard(challenges.at(i));
          ...
          for (const [u, sr] of Object.entries(obj)) {
            ...
            if (!e) {
              leaderboard.push({ name: u, rank: r, challenges: 1 })
            } else {
            ...
            <table className="min-w-full text-white border border-[#DEF400] border-opacity-90	 rounded-lg">
              <thead>
                <tr >
                  ...
                  <th className="py-2 px-4 border-[#DEF400] border-b">Challenges</th>
                </tr>
              </thead>
              <tbody>
                {
                  loading ? (
                    ...
                    <tr className='text-center border-opacity-90' key={index}>
                      ...
                      <td className="py-2 px-4 border-[#DEF400] border-b">{entry.challenges}</td>
                    </tr>
                  ))}
              </tbody>
            </table>
